...
## 2026-09-24 - MCP SDK v2 compatibility migration, issue #3

Added the SDK v2 request factory on `/mcp`, retaining the legacy `/mcp` and `/sse` lanes until clients migrate and sessions drain. The shared tool factory accepts an injected server constructor, preserving stdio's SDK v1 default. Owner-approved per-connection defaults use authenticated identity and existing application-state storage; deletion clears them. No production configuration or persisted legacy session data is changed. Local fixtures test real connector OAuth and both transport generations without Wave traffic. See worker/README.md for receiving-host verification and explicit legacy retirement gates.
